...
-- y' = f(x,y), y(x0) = y0, por el metodo de Heun
...
local function solve(expr, x0, y0, h, n)
    ...
    for i = 0, n - 1 do
        ...
        if not f1 then return nil, nil, nil, e1 end
        local xnext = xs[i] + h
        local ypred = ys[i] + h * f1
        local f2, e2 = feval(expr, xnext, ypred)
        if not f2 then return nil, nil, nil, e2 end
        local ynext = ys[i] + (h / 2) * (f1 + f2)
        log[#log + 1] = string.format("Paso %d:", i + 1)
        log[#log + 1] = string.format("  f1=f(x%d,y%d)=f(%.4f,%.4f)=%.6f", i, i, xs[i], ys[i], f1)
        log[#log + 1] = string.format("  yp = y%d + h*f1 = %.4f + %.4f*%.6f = %.6f", i, ys[i], h, f1, ypred)
        log[#log + 1] = string.format("  f2=f(x%d,yp)=f(%.4f,%.4f)=%.6f", i + 1, xnext, ypred, f2)
        log[#log + 1] = string.format("  y%d = y%d + (h/2)(f1+f2)", i + 1, i)
...
function on.paint(gc)
    ...
    gc:drawString("EDO: metodo de Heun", 2, 2, "top")

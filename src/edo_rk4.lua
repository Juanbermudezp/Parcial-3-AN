...
-- y' = f(x,y), y(x0) = y0, por el metodo de Runge-Kutta 4
...
local function solve(expr, x0, y0, h, n)
    ...
    for i = 0, n - 1 do
        ...
        log[#log + 1] = string.format("  k1=f(%.4f,%.4f)=%.6f", xs[i], ys[i], k1)
        log[#log + 1] = string.format("  k2=f(%.4f,%.4f)=%.6f", xm, ym1, k2)
        log[#log + 1] = string.format("  k3=f(%.4f,%.4f)=%.6f", xm, ym2, k3)
        log[#log + 1] = string.format("  k4=f(%.4f,%.4f)=%.6f", xnext, yp4, k4)
        log[#log + 1] = string.format("  y%d = y%d + (h/6)(k1+2k2+2k3+k4)", i + 1, i)
...
function on.paint(gc)
    ...
    gc:drawString("EDO: metodo de Runge-Kutta 4", 2, 2, "top")
